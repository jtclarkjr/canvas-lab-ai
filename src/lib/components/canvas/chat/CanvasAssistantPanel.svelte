...
{#if store.assistantInitialMessages !== null}
  <!-- Keyed so switching canvases rebuilds the Chat instance with the new
       thread's history. -->
  {#key canvasId}
    <CanvasAssistantThread {canvasId} initialMessages={store.assistantInitialMessages} />
  {/key}
...
{:else}
  <div class="flex h-full flex-col justify-end gap-3 px-4 py-4" aria-hidden="true">
