...
<Drawer
  bind:open
  title="Call sessions"
  description="Call summaries and transcripts"
  widthClass="w-[min(44rem,100vw)]"
  layerClass="z-40"
  closeLabel="Close call sessions"
  class="bg-card backdrop-blur-none"
>
  {#snippet headerActions()}
    <IconButton
      label="Refresh call sessions"
      variant="ghost"
      ...
      {:else}
        <div class="mb-4 flex items-start justify-between gap-4">
          ...
          >
            {#if selectedSession.transcriptStatus === 'ready'}
              <CircleCheck class="mr-1 inline size-3.5" aria-hidden="true" />
        ...
        {:else}
          ...
          {:else if groupedSegments.length === 0}
            ...
            >
              {selectedSession.errorMessage ?? 'No transcript lines captured.'}
            </div>
          {:else}
            <div class="flex flex-col gap-5">
              {#each groupedSegments as utterance (utterance.id)}
                <article class="border-b border-border/70 pb-5 last:border-0">
                  <div
