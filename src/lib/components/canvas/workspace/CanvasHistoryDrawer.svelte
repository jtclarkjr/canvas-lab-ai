...
<Drawer
  ...
  description="Element activity for this canvas"
  closeLabel="Close canvas history"
>
  {#snippet headerActions()}
    <IconButton
      label="Refresh canvas history"
      variant="ghost"
    ...
    {#if isLoading && entries.length === 0}
      <div class="grid gap-2 px-3 py-3" aria-hidden="true">
        {#each Array.from({ length: 6 }) as _, index (index)}
          ...
          >
            <Skeleton shape="circle" class="size-8 bg-secondary" />
            <div class="min-w-0 flex-1">
              <Skeleton shape="text" class="w-2/3 bg-secondary" />
              <Skeleton shape="text" class="mt-2 h-3 w-1/2 bg-secondary/80" />
            </div>
          </div>
        {/each}
      </div>
