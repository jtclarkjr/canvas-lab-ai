...
<Card
  variant="glass"
  interactive={touchLike || interactive}
  class={`group ${touchLike ? 'pointer-events-auto' : interactive ? 'pointer-events-auto' : 'pointer-events-none'} absolute flex cursor-pointer flex-col overflow-hidden p-3 text-left transition-shadow active:scale-[0.99]`}
>
  {#snippet child({ props })}
    <div
      {...props}
