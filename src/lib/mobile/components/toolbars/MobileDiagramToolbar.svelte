<script lang="ts">
  ...
  let mobileOptionsExpanded = $state(false)
  ...
  const mobileToolbarClass = $derived(
    mobileOptionsExpanded
      ? 'pointer-events-auto fixed inset-x-2 bottom-[calc(env(safe-area-inset-bottom)+4.75rem)] z-40 flex max-h-[min(26rem,54dvh)] flex-wrap items-center gap-1 overflow-y-auto rounded-2xl border border-border/70 bg-card/95 p-2 text-card-foreground shadow-2xl backdrop-blur transition-all'
      : 'pointer-events-auto fixed left-3 bottom-[calc(env(safe-area-inset-bottom)+4.75rem)] z-40 flex rounded-full border border-border/70 bg-card/95 p-1.5 text-card-foreground shadow-2xl backdrop-blur transition-all'
  )
  ...
  onMount(() => {
    const handlePointerDownOutside = (event: PointerEvent) => {
      if (
        mobileOptionsExpanded &&
        toolbarEl &&
        !event.composedPath().includes(toolbarEl)
      ) {
  ...
  function buttonClass(active: boolean) {
    return `flex h-10 w-10 items-center justify-center rounded-lg transition ${
      active
  ...
  function swatchClass(active: boolean) {
    return `size-7 rounded-md border transition ${
...
{#if isVisible}
  <div
    bind:this={toolbarEl}
    class={mobileToolbarClass}
    data-diagram-toolbar
    data-camera-exempt
  >
    <button
      type="button"
      class={buttonClass(mobileOptionsExpanded)}
      onmousedown={preventEditorBlur}
      onclick={() => (mobileOptionsExpanded = !mobileOptionsExpanded)}
      aria-label={mobileOptionsExpanded
        ? 'Collapse diagram options'
      ...
      {#if onTemplateInsert}
        ...
        >
          ...
          {#if isTemplateMenuOpen}
            <div
              class="fixed inset-x-4 bottom-[calc(env(safe-area-inset-bottom)+13rem)] rounded-lg border border-border/70 bg-popover text-popover-foreground shadow-xl"
            >
              <DiagramTemplateMenu
